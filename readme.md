...
* **JSON Processing**: Securely decode requests with size limits, encode responses with custom headers and status codes and sends errors through JSON.
...
func JSONHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if err := t.ReadJSON(w, r, &payload); err != nil {
        // Status code http.StatusBadRequest set as default.
        t.ErrorJSON(w, err.Error())
        return
    }
...
* **`ErrorJSON`**: Encodes an error response into JSON, sets the `Content-Type`, and writes headers.
